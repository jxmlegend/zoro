...
********************************************/
 ...
 //变量、常量定义
float rp=1,gp=2,bp=1.75; //定义比例因子，具体环境可以修改
u16 red_dat,green_dat,blue_dat;
...
void TIM4_Configuration(void)//只用一个外部秒冲端口
{
  ...
  GPIO_InitTypeDef  GPIO_InitStructure;
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB|RCC_APB2Periph_AFIO, ENABLE);	 //使能PA端口时钟
	

	GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable, ENABLE);//关闭jtag，使能SWD，可以用SWD模式调试
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0|GPIO_Pin_3;	// PB0、PB1端口配置
  ...
  GPIO_ResetBits(GPIOB,GPIO_Pin_0|GPIO_Pin_3);
//第二步：配置定时器
