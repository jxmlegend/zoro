...
void SYN_TTS(uint8_t *Pst)
{
	...
	Send_InfoData_To_Fifo("#S1#",4);
	Send_InfoData_To_Fifo(&Back[3],1);
	SYN7318_Get_String(Back,4);	
	//接收成功
	while(!(Back[3] == 0x41))
	{
		Send_InfoData_To_Fifo("#S2#",4);
		Send_InfoData_To_Fifo(&Back[3],1);
		SYN7318_Get_String(Back,4);
	...
	//空闲监测
	Send_InfoData_To_Fifo("#S3#",4);
	SYN7318_Get_String(Back,4);
	while(!(Back[3] == 0x4f))
	{
		Send_InfoData_To_Fifo("#S4#",4);
		SYN7318_Get_String(Back,4);
	}	
}
				...
				if(ASR[0] ==0x01) 
				{
					switch(ASR[5]) // 命令ID
					{
						case 0x19:   //  原地掉头
					    {
							  ...
							  SYN_TTS("识别成功原地掉头");
							  SYN7318_Put_String(Stop_ASR_Buf,4); //停止语音识别		
								Send_ZigbeeData_To_Fifo(terminal_buf,8);//发送语音识别结果给终端
							  Ysn7813_flag =0;
							 break;
						}														
						case 0x21:   //  向右转弯
