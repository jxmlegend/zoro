...
函数功能；RFID读写卡
r_w；0 写  1 读
block——no：块地址(0~63)  十六进制 （0~3F）
其中 3，7，B,F,13,17,1B,1F,23,27,2B,2F,33,37,3B,3F
块为秘钥和控制块
*****************************************************************/
u8 RW_Card(u8 r_w,u8 block_no,u8 *data_array_addr)
{
	char status = 0;
//	u8 CT[2];									//卡类型
	u8 SN[4]; 									//卡号
	//u8 KEY[6]={0xff,0xff,0xff,0xff,0xff,0xff};  //秘钥
	//u8 KEY[6]={0xA0,0xA1,0xA2,0xA3,0xA4,0xA5};  //秘钥
	u8 KEY[6]={0xD3,0xF7,0xD3,0xF7,0xD3,0xF7};  //秘钥
	u8 s = block_no;     							// #define   Status2Reg   0x08	
//	u8 RXRFIDH[8];
		...
		if(status == MI_OK)							//寻卡成功
		{
			...
			if(status == MI_OK)
			{
				...
				if(status == MI_OK)					//????
				{
					...
					status =PcdAuthState(0x60,(block_no/4)*4+3,KEY,SN);		//????
					if(status == MI_OK)
					{
						status = MI_ERR;
					  if(!r_w)
						{
							status = PcdWrite(s,data_array_addr);			//??
//							if(status == MI_OK)
//							{
//								status = MI_ERR;
//							}	
						...
						{
							status=PcdRead(s,data_array_addr);				//??
//							if(status == MI_OK)
//							{
//								status = MI_ERR;							
//							}
...
void RC522_Uart_init(u32 baudrate)
{
	...
	NVIC_TypeDefStructure.NVIC_IRQChannelSubPriority = 5;
